...
function App() {
  ...
  // optimizedHeavyComputationOnEachItem executes, only when "posts" changes 
  // you can increase and decrese counter to test. 
  // it won't get executed with "count" or any other state variables except "posts"
  ...
  // if you want to rerender it on a particulat state or props change, then pass that to dependecy array of useCallback
  ...
  return (
    <div className="container">
      <div className="button-container">
        ...
        {count}
        <button onClick={() => setCount(count + 1)}>+</button>
        <button onClick={() => setSelectedPostId(null)}>Reset Post Selection</button>
      </div>
      {selectedPostId && (
        <PostDetails
      ...
      <div className="button-container">
        {
          ...
          <button
            onClick={() => fetchPosts(paginationLinks[`rel="prev"`])}
          >
            Previous
          </button>
        }
        {
          ...
          <button
            onClick={() => fetchPosts(paginationLinks[`rel="next"`])}
          >
            Next
          </button>
        }
